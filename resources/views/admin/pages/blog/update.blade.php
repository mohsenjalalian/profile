            ...
            <form id="contact" action="{{route('blog.update',$blog->id)}}" method="post"
                ...
                <fieldset>
                    <div class="form-group{{ $errors->has('title') ? ' has-error': ''}}">
                        <fieldset>
                            <input class="form-control m-b col-md-4" placeholder="تیتر" type="text" value="{{$blog->title}}" name="title" tabindex="1" required autofocus>
                        </fieldset>
                        @if($errors->has('title'))
                    ...
                    <div class="form-group{{ $errors->has('description') ? ' has-error': ''}}">
                        <fieldset>
                            <textarea style="height: 80px !important;" class="form-control m-b col-md-4" name="description" rows="8" cols="80" placeholder="توضیحات" tabindex="1" required>{{$blog->description}}</textarea>
                        </fieldset>
        ...
        <div class="ibox float-e-margins">
            <div class="form-group{{ $errors->has('photo') ? ' has-error': ''}}">
                @if(!empty($blog->album[2]->photo))
                    <input type="file" id="file3" name="photo" value="{{$blog->album[2]->photo}}" onchange="readURL3(this)" style="display:none" >
                    <img style="width: 50px;height: 50px" id="photo3" src="{{asset(\App\Http\Controllers\BlogController::ALBUM_PATH.'/'.$blog->album[2]->photo)}}" alt="{{$blog->album[2]->photo}}">
                @else
